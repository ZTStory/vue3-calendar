...
export default defineComponent({
    ...
    setup() {
        ...
        watch(selectedDates, (newVal) => {
            if (newVal.length) localStorage.setItem("CalendarSelect3", newVal[0]);
            else localStorage.setItem("CalendarSelect3", "");
            backAction();
        });
        ...
        config.settingDayCb = (day: ZTCalendarDay) => {
            ...
            if (currentDay.isSame(DayjsUtils.add(4), "day") || currentDay.isSame(DayjsUtils.add(11), "day")) {
                day.isWork = true;
            }
            return day;
        };
